...
public final class CmdRunner {
    ...
    public static GeneralCommandLine createCommandLine(@NotNull String cwd, String node, String exe) {
        cwd = Paths.get(cwd).normalize().toAbsolutePath().toString();
        exe = Paths.get(exe).normalize().toAbsolutePath().toString();
        node = Paths.get(node).normalize().toAbsolutePath().toString();
     ...
     */
    @NotNull
    public static ProcessOutput execute(@NotNull GeneralCommandLine commandLine, com.intellij.execution.process.ProcessListener listener, int timeoutInMilliseconds) throws ExecutionException {
        ...
        if (processHandler.waitFor(timeoutInMilliseconds)) {
            System.out.println("CmdRunner execute end");
            output.setExitCode(process.exitValue());
        } else {
            processHandler.destroyProcess();
            output.setTimeout();
            System.out.println("CmdRunner execute timetou");
        }
        if (output.isTimeout()) {
    ...
    public static ProcessOutput execute(@NotNull GeneralCommandLine commandLine, int timeoutInMilliseconds) throws ExecutionException {
        return execute(commandLine, (ProcessListener) null, timeoutInMilliseconds);
    }
}